...
# Preallocate values
Ron_UW_beta = np.zeros([len(Rona.columns)-25,2])
Ron_UW_r    = np.zeros([len(Rona.columns)-25,1])
...
for i in range(11,len(Rona.columns)-14):
	j=i-11 # save indexing
	
	# Get Corona change over past 14 days
	RonaChange = RonaRed[Rona.columns[i+14]]-RonaRed[Rona.columns[i]]
	...
	DeatChange =  DeatRed[Deat.columns[i+14]] - DeatRed[Deat.columns[i]]
...
def PlotDeatDay(i, D=14):
	...
	plt.title('New Deaths from ' +Deat.columns[i] + ' to ' + Deat.columns[i+D]+ ' vs. 2016 Election, Alaska Excluded')
